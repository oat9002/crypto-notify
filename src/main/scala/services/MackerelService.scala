...
class MackerelServiceImpl(configuration: Configuration, httpClient: HttpClient)(implicit system: ActorSystem[Nothing], context: ExecutionContext) extends MackerelService with LazyLogging {
  override def sendMeasurement(request: List[MackerelRequest]): Future[Boolean] = {
    ...
    val response = httpClient.post[List[MackerelRequest], Any](url, request, Map("X-Api-Key" -> s"${configuration.mackerelConfig.apiKey}"))
